function [  ] = SortImages(I , image_name)
% Detects and extracts faces from image I. If a label is detected
...
% while keeping its original file name. 
    ...
    % Limit the white regions to ones which have an area between 150 and 
    % 2000, and a solidity of under 0.7
    stats = regionprops(WB, 'Area', 'Solidity');
    CC = bwconncomp(WB);
    L = labelmatrix(CC);
    BW2 = ismember(L, find([stats.Area] <= 2000 & [stats.Area] > 150 & [stats.Solidity] < 0.7));
    ...
    % to height ratio between 0.35 and 1.5, a width between 15 and 40, a
    % height between 20 and 50, and a location within the middle 70% of the
    % x-axis.
    aspectRatio = width ./ height;
    ratioConstraint = aspectRatio < 1.5 & aspectRatio > 0.35;
    shapeConstraint = width < 40 & width > 15 & height < 50 & height > 20;
    ...
    if blobs_found >= 2
        ...
        if label_found
            ...
            for x = 1:r
                ...
                imwrite(FF, sprintf('Faces/%s/%s', label, image_name));
                clear FF;
            end
        else
            % no label found
            for x = 1:r
                FF = I(BBOX(x,2):BBOX(x,2)+BBOX(x,4), BBOX(x,1):BBOX(x,1)+BBOX(x,3), :);
                imwrite(FF, sprintf('Faces/Not Found/%s', image_name));
                clear FF;
            end
        end
    else
